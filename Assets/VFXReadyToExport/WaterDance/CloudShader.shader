Shader "Unlit/CloudShader"
{
	Properties
	{
		...
		_CloudColorTransition("Cloud Color Transition", Range(0, 1)) = 0
		...
		_DissolveProgress("Dissolve Progress", Range(0, 1)) = 0
			...
			_NoiseRumbleSpeed("Noise Rumble Speed", Float) = 10
			_NoiseRumbleStrength("Noise Rumble Strength", Float) = 0.2
		...
		Tags
{
	...
	"PassFlags" = "OnlyDirectional"
		...
		Pass
		{
			...
			float _NoiseRumbleSpeed;
			float _NoiseRumbleStrength;
			...
			v2f vert (appdata v)
			{
				...
				v.vertex.xyz += _NoiseRumbleStrength * normalize(o.worldNormal) * tex2Dlod(_DissolveTex, float4(o.uv.xy + float2(_Time.x * _NoiseRumbleSpeed, 0), 0.0, 0.0)).r;




				//v.vertex.xyz += float3(0, unity_DeltaTime.x, 0);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float NdotL = abs(_WorldSpaceCameraPos.xyz);//dot(_WorldSpaceLightPos0, normal);
				...
				return _Color * col * (/*_AmbientColor*/ (0.4 + 0.6 * _CloudColorTransition) + /*light +*/ specular + rim + IriColor * _IriMag);
			}
			ENDCG
		}
