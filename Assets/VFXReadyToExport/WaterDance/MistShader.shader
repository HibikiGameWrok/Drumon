Shader "Unlit/MistShader"
{
	Properties
	{
		...
		_DissolveProgress("Dissolve Progress", Range(0, 1)) = 0
			...
			_BulgerMagnifier("Bulger Magnifier", Range(-2, 2)) = 0.2
	...
	"Queue" = "Transparent"
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				v.vertex.xyz += _BulgerMagnifier * normalize(o.worldNormal) * tex2Dlod(_DissolveTex, float4(o.uv.xy + float2(_Time.x * 10, 0), 0.0, 0.0)).r;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				return _Strength * _Color * col * (/*_AmbientColor*/ (0.4 + 0.6 * _CloudColorTransition) + rim + IriColor * _IriMag);
			}
			ENDCG
		}
