Shader "Unlit/WaterImpactConstantShader"
{
	Properties
	{
		...
		_HeightChangeSpeed("Height Change Speed", Float) = 1
		...
		SubShader
		{
			...
			Pass
			{
				...
				float MaxHeight = tex2D(_MainTex, float2(i.uv.x, _UVYHeight + _HeightChangeSpeed * _Time.x)).r * _Magnification * abs(sin(3.14 * (_Progress))) /*+ 2 * (0.1 * sin(5 * _Time.z)) - 0.2*/;
				...
				col *= smoothstep(0.0, 0.4, i.HeightValue);
